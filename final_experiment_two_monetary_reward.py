...
buffer = 40
training_reps = 300
test_reps = 50 # 50*6 conditions = 300 trials
test_phase_waiting_time = 10 # in seconds.
...
n_training_trials = len(trial_conditions_training)
# Precompute a potential increment for each trial (non-linear & unpredictable)
potential_increments = np.random.rand(n_training_trials)
...
for thisTrial in training_trials:
    ...
    if square_color == high_value_color:
        ...
        # Award reward using the next potential increment
        this_increment = potential_increments[reward_counter]
        reward_counter += 1  # move to the next potential increment
        awarded_increments.append(this_increment)
        provisional_total += this_increment

        # Display provisional cumulative reward (it won’t exactly sum to 150 yet)
        reward_text = visual.TextStim(win, 
                                      text=f"Total money earned: Rs. {provisional_total:.2f}", 
                                      pos=(0, 0), color='white')
        reward_text.draw()
        win.flip()
        ...
        thisExp.addData('colour_condition', 'low_value_color')
        thisExp.addData('square_color', low_value_color)
        # No reward is given on low_value_color trials.
        trigger = "no_reward"
...
final_reward_text = visual.TextStim(win, 
                                    text=f"Your final reward is: Rs. 150.00", 
                                    pos=(0, 0), color='white')
final_reward_text.draw()
win.flip()
